#!/usr/bin/env python3
"""
编译并安装 jsoncpp 到 teamtalk/.sdk/jsoncpp/ 目录
...
使用方法:
    ...
    python3 build_jsoncpp.py clean     # 清理构建结果
    python3 build_jsoncpp.py all       # 完整流程：清理+编译+安装（默认）
"""
...
class JsoncppBuilder:
    ...
    def is_built(self):
        """检查是否已经编译过"""
        # 检查编译产物是否存在
        lib_file = self.build_dir / "src" / ".libs" / "libjsoncpp.so"
        if not lib_file.exists():
            # 尝试其他可能的文件名
            lib_file = self.build_dir / "src" / ".libs" / "libjsoncpp.dylib"
        if not lib_file.exists():
            lib_file = self.build_dir / "src" / ".libs" / "libjsoncpp.a"
